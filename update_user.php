...
<body>
  ...
  <div class="container bg-light text-dark" style="opacity:95%;">
     ...
     <?php
        ...
        $get_user = $mysqli->query($query_user);
        $user = mysqli_fetch_assoc($get_user);
     ?>
     <div style="padding-bottom:30px;"></div>
     <form action="" method="POST">
       <div style="border:2px solid #333;  margin-left:25%; margin-right:25%; padding-bottom:30px; background-color:#FFB266; border-radius:30px;">
        <div class="h1 p-1 pt-4 text-center">Update User</div>
    ...
    <?php
      if(isset($_POST['submit'])) {
        ...
        if ($password == $password_confirmation){
              ...
              $email_id = mysqli_fetch_assoc($unique_email);
              // Check if email is unique, or return an error
              if (($unique_email->num_rows == 1  && $email_id['id'] ==$user['id']) || $unique_email->num_rows ==0) {

                  $query = "UPDATE users SET first='".$first_name."', last='".$last_name."', email='".$email."',
                            password='".$password."', role_id='".$user_type."'
                            WHERE id='".$id."'";

                  $added_user = $mysqli->query($query);
                  // Final validation that the query succeeded
                  if($added_user){
                      setcookie("updated_user", "User has been successfully updated", time() + (86400 * 30), "/");
                      header("location: admin_view.php",  true,  301 );
                      exit();
                  }
              } else { ?>
